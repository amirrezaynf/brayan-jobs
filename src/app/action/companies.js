...
function mapFormDataToAPI(formData) {
  return {
    ...
    services: Array.isArray(formData.services) ? formData.services.filter(s => s.trim()) : [],
    technical_specialties: Array.isArray(formData.specialties) ? formData.specialties.filter(s => s.trim()) : [],
    benefits: Array.isArray(formData.benefits) ? formData.benefits.filter(b => b.trim()) : [],
...
export async function getCompanies(params = {}) {
  try {
    ...
    if (params.activity_field_id) queryParams.append('activity_field_id', params.activity_field_id);
    if (params.verified !== undefined) queryParams.append('verified', params.verified);
    if (params.founded_year_from) queryParams.append('founded_year_from', params.founded_year_from);
    if (params.founded_year_to) queryParams.append('founded_year_to', params.founded_year_to);
 ...
 * آپلود تصویر شرکت
 */
export async function uploadCompanyImage(companyId, imageFile, imageType = 'logo') {
  try {
