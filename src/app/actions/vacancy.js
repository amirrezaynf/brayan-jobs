...
async function getAuthToken() {
  try {
    ...
    try {
      ...
      console.log(
        "⚠️ Could not access cookies (might be client-side):",
        cookieError.message
      );
    }
    ...
    if (typeof window !== "undefined") {
      ...
      console.log(
        ...
        localToken ? `${localToken.substring(0, 10)}...` : "null"
      );
      return localToken;
    }
...
function mapFormDataToAPI(formData) {
  ...
  const mappedData = {
    ...
    ...(formData.salary &&
      parseSalary(formData.salary) && { salary: parseSalary(formData.salary) }),
  ...
  console.log(
    "  - description:",
    typeof mappedData.description,
    mappedData.description
  );
  console.log(
    "  - contract_type:",
    typeof mappedData.contract_type,
    mappedData.contract_type
  );
  console.log("  - salary:", typeof mappedData.salary, mappedData.salary);
  console.log(
    "  - expires_at:",
    typeof mappedData.expires_at,
    mappedData.expires_at
  );
  console.log(
    "  - is_remote_possible:",
    typeof mappedData.is_remote_possible,
    mappedData.is_remote_possible
  );
  console.log(
    "  - is_urgent:",
    typeof mappedData.is_urgent,
    mappedData.is_urgent
  );
...
function saveVacancyToLocalStorage(vacancy) {
  try {
    const existingVacancies = JSON.parse(
      localStorage.getItem("userVacancies") || "[]"
    );
...
export async function createVacancy(vacancyData, clientToken = null) {
  try {
    ...
    console.log(
      ...
      clientToken ? `${clientToken.substring(0, 10)}...` : "null"
    );
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (storedCompanyData) {
          ...
          if (!companyData.id) {
            console.log(
              "⚠️ Company exists in localStorage but has no ID - might not be saved to server"
            );
          }
        } else {
    ...
    try {
      const testResponse = await fetch(
        `https://imocc.iracode.com/api/v1/companies`,
        {
          method: "GET",
          headers: {
    ...
    if (typeof window !== "undefined") {
      ...
      if (fallbackResult.success) {
        return {
          ...
          message:
            "آگهی‌ها از حافظه محلی بارگذاری شدند (اتصال به سرور برقرار نشد)",
        };
      }
    }
