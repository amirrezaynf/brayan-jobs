...
// Client-side API functions for vacancy management
...
function mapFormDataToAPI(formData) {
  ...
  const mappedData = {
    ...
    company_id: 1, // Default company ID - باید از user profile گرفته بشه
...
// Create new vacancy - Client-side function with retry
export async function createVacancyClient(vacancyData, retryCount = 0) {
  ...
  try {
    ...
    // After all retries failed, fallback to localStorage
    console.log("🔄 All retries failed, falling back to localStorage...");
    const fallbackResult = saveVacancyToLocalStorage(vacancyData);
    if (fallbackResult.success) {
      return {
        success: true,
        data: fallbackResult.data,
        message: "آگهی به صورت محلی ذخیره شد. وقتی اتصال برقرار شد، به سرور ارسال خواهد شد.",
      };
    }
    ...
    if (error.name === 'AbortError') {
      ...
      return {
        success: false,
        error: "خطا در اتصال به سرور. آگهی به صورت محلی ذخیره شد.",
      };
    }
